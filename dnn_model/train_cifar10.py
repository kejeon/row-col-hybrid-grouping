...
parser.add_argument('--optimizer', default='adam', help='optimizer function used')
...
parser.add_argument('--weight_bit_width_list', default='2,4,16,256,0', help='weight bit width list')# 2,4,16,256,0
parser.add_argument('--acti_bit_width_list', default='4', help='activation bit width list')
...
def main():
    ...
    weight_bit_width_list = list(map(int, args.weight_bit_width_list.split(',')))
    acti_bit_width_list = list(map(int, args.acti_bit_width_list.split(',')))

                                        # acti_bit_width_list
    model = models.__dict__[args.model](acti_bit_width_list, weight_bit_width_list, train_data.num_classes).cuda()
    ...
    if args.resume and args.resume != 'None':
        ...
        if os.path.isfile(args.pretrain):
            ...
            # Added by IRIS Lab. 
            # Only load parameters from the FP32 layers
            my_keys = list(checkpoint['state_dict'].keys())
            for i in my_keys:
                if 'bn_dict' in i:
                    if 'bn_dict.32' in i:
                        checkpoint['state_dict'][i.replace('bn_dict.32', 'bn_dict.0')] = checkpoint['state_dict'][i]
            ...
            for name, param in model.named_parameters():
                if 'conv' in name or 'fc' in name or 'bn_dict.0' in name:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if sum_writer is not None:
            ...
            for bw, tl, tp1, tp5, vl, vp1, vp5 in zip(weight_bit_width_list, train_loss, train_prec1, train_prec5, val_loss,
                                                      val_prec1, val_prec5):
                ...
                logging.info('weight bit precision: {}, accruacy: {}'.format(bw, vp1))
...
def forward(data_loader, model, criterion, criterion_soft, epoch, training=True, optimizer=None, sum_writer=None):
    weight_bit_width_list = list(map(int, args.weight_bit_width_list.split(',')))
    acti_bit_width_list = list(map(int, args.acti_bit_width_list.split(',')))
    # weight_bit_width_list.sort()
    # acti_bit_width_list.sort()


    losses = [AverageMeter() for _ in weight_bit_width_list]
    top1 = [AverageMeter() for _ in weight_bit_width_list]
    top5 = [AverageMeter() for _ in weight_bit_width_list]
    ...
    for i, (input, target) in enumerate(data_loader):
        if not training:
            with torch.no_grad():
                ...
                for bw, am_l, am_t1, am_t5 in zip(weight_bit_width_list, losses, top1, top5):
                    model.apply(lambda m: setattr(m, 'wbit', bw))
                    model.apply(lambda m: setattr(m, 'k', bw))
                    model.apply(lambda m: setattr(m, 'abit', acti_bit_width_list[-1])) #acti_bit_width_list[-1]
                    output = model(input)
            ...
            model.apply(lambda m: setattr(m, 'wbit', weight_bit_width_list[-1]))
            model.apply(lambda m: setattr(m, 'k', weight_bit_width_list[-1]))
            model.apply(lambda m: setattr(m, 'abit', acti_bit_width_list[-1])) # acti_bit_width_list
            output = model(input)
            loss = criterion(output, target)
            loss.backward()
            prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
            ...
            for bw, am_l, am_t1, am_t5 in zip(weight_bit_width_list[:-1][::-1], losses[:-1][::-1], top1[:-1][::-1],
                                              top5[:-1][::-1]):
                model.apply(lambda m: setattr(m, 'wbit', bw))
                model.apply(lambda m: setattr(m, 'k', bw))
                model.apply(lambda m: setattr(m, 'abit', acti_bit_width_list[-1])) # acti_bit_width_list[-1]
                ...
                am_t1.update(prec1.item(), input.size(0))
                am_t5.update(prec5.item(), input.size(0))
                # print("weight bit precision: {}, accruacy: {}".format(bw, prec1))
            optimizer.step()
