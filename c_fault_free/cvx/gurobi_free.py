...
class BaseGurobiOptimizer:
    ...
    def _setup_constraints(self) -> None:
        ...
        self.x_corrupt: gp.MVar = x_corrupt
        ...
        # self.saf_0_pos_constr = self.model.addConstr(self.saf0_pos == np.zeros((self.C, self.R)))
        # self.saf_0_neg_constr = self.model.addConstr(self.saf0_neg == np.zeros((self.C, self.R)))
        # self.q_code_constr = self.model.addConstr(self.q_code == np.zeros((self.R - self.R_start,)))
        self.model.update()
...
class GurobiOptimizer(BaseGurobiOptimizer):
    ...
    def setup_problem(self, fault_pos: np.ndarray, fault_neg: np.ndarray,
        ...
        self.x_ub.rhs = (both_no_fault | only_pos_fault).astype(int) * (self.mem_q_lvl - 1)
        self.x_lb.rhs = -(both_no_fault | only_neg_fault).astype(int) * (self.mem_q_lvl - 1)
        self.model.update()
